...
SQL is a language that communicates with a database. RDBMS stands for 'relational database management system', and it's the basis for SQL and all modern database systems.

1. Why do tables need a `primary key`?
So they can have a unique identifier for each item so they can always reference each item individually and specifically.

1. What is the name given to a table column that references the primary key on another table.
Foreign key

1. What do we need in order to have a _many to many_ relationship between two tables.
In some data relationships, many pieces of data can reference many others in specific ways, but not so specific that deleting one of these data would affect any of the others. For example, in a restaurant, customers relate to their orders in that many customers can have many different orders, and many orders can be related to the multiple customers who ordered them.
